...
7. Do the work.
8. Leave a task comment and update task status. If you discover new distinct sub-tasks or blockers during execution, use `create_task` to add them to the board rather than hiding them inside a comment.
...
4. Move the task status with `update_task` so the board matches reality. If you are not sure of the exact status string, check the project's workflow columns first.
